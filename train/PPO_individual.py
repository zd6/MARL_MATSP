...
def main():
    ...
    max_steps = 10  #int(env.flag_num*40/env.agent_num/step_length)      # max timesteps in one episode
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        if i_episode % log_interval == 0:
            ...
            np.save('logging_9_indi_lr_0_001_scheduler_step10_f{0}a{1}.npy'.format(flag_num, agent_num), logging_file)
            print('Episode {} \t avg length: {} \t reward: {} \t error: {} \t current_lr: {}'.format(i_episode, avg_length, running_reward, avg_error, ppo.scheduler.get_last_lr()))
            running_reward = 0
            avg_length = 0
            avg_error = 0
    torch.save(ppo.policy.state_dict(), './PPO_indi_{}.pth'.format('matsp-v0'))
